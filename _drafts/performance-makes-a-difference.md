...
Now getting these scores didn't cost me a single penny, but some time and dedication. Hopefully by the end of this post you'll be able to apply some of these tips to your company's site, then your boss will give you a raise and you'll be happy. 

## Does performance matter?
If you're not worried about performance, let me give you a couple of reasons to do so:  
There's been lot of discussions lately about [Facebook Instant Articles](https://www.youtube.com/watch?v=zPvGF5bO0xs). With this, Facebook is avoiding hitting the website and getting directly to the content just because the web _is_ too slow. But that's our fault, and you can help get this situation better. I had a [discussion with Scott Jehl](https://twitter.com/scottjehl/status/598860119628238848) about SpeedIndex and it is clear that a <3000 SpeedIndex on mobile 3G is kind of a low bar for what the web is up against.

Not only that, but has been proven that users expect mobile web to be fast. As stated in [Designing for Performance](http://larahogan.me/design/):

> "A user expects a site to load on a mobile device as fast if not faster than in a desktop device" - Lara Hogan

This is given probably because the handheld devices are closer and may seem "simpler" than desktop devices, just because they show... less stuff on screen. But considering responsive design implies serving the same HTML to every device, this isn't really what happens, just on a different way.
...
- [theguardian's mq](https://github.com/sass-mq/sass-mq): Media queries couldn't be easier and more consistent.
...
- Do you really need all those font files? Are you using all the font weight? Even if you do, do you use them that often? Try changing unique font weights to closer, more common ones.
- That hero image slider, do users click on the "next" button of the slider. Studies show users don't do that. Track it and make sure you need all those super big hero images.
- Are you hiding stuff on mobile? If so and it has images, have you made sure they're not loading on mobile? If they're not shown, they're not needed. Use Chrome Dev Tools' emulator and check the Network panel.
- If using any third party content, are they requesting other libraries that affect your site's performance? Contact them and get to a middle ground or try lazy load them when possible.
...
I hope you learned something today. If you're using WordPress, chances are you can get a <3000 SpeedIndex in no time with these tips. We can make the web faster, so we should. Go do your bit. It'll be better for everybody's roaming bill.
